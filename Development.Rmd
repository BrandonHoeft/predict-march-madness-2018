...
head_to_head_stats <- reg_season_details_cumulative_stats %>%
    ...
    # Select variables
    select(game_id, Season, DayNum, TeamID, Team_name, Team_conf, outcome, outcome_1_0,
           ...
           opponent_ORB_rate, opponent_DRB_rate, average_tempo, season_wins,
           season_win_rate, ends_with("OPP")) %>%
...
head_to_head_matchup_stats <- head_to_head_stats %>%
    mutate(possessions_DIFF = possessions - possessions_OPP, # basically the same as tempo_DIFF
...
* Filter out features that aren't goin to be considered as predictors. Only need the target vector and feature vectors.
* apply a **sampling strategy**: make sure the data is balanced and records are independent. For instance, there should be about 50/50 proportion of W's and L's to predict, and no unique game should be included in the modeling dataset more than once.  
