...
*  Matchup Differentials: Algorithms that treat bracketology as a ranking problem assume that the transitive property applies to head to head match ups for teams in the tournament that likely haven't played each other before or often. If team A is > team B on an index score, and team A < C on this scoring system, then team C > team B and we predict team C to win. Generally, this may be a good heuristic[probably holds up](https://www.theplayerstribune.com/math-meets-football-the-transitive-property-is-real/), but I'm venturing that this is problematic for modeling the potential of an **upset**, because we're not considering how well team A and team C match up individually with each other. For instance, the absolute difference in the respective shooting efficiency of team C from team B heading into a game, and controlling for or interacting with other predictors, may account for match up differences in a head to head contest that actually gives team B the edge over team C that a simple composite score ignores. 
