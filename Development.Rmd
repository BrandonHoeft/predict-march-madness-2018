...
reg_season_details_cumulative_stats2 %>%
    filter(Team_name == "Loyola-Chicago" & Season == 2017) %>%
    select(1:8, 24:45) %>%
     View()
...
modeling_dataset <- head_to_head_matchup_stats %>%
    # only consider games starting after the quarter mark of the season.
    filter(DayNum > quantile(RegularSeasonDetailedResults$DayNum, .25)) %>%
