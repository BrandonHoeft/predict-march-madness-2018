...
## Modeling Dataset 2003-2014 (to Predict 2014-2017 Tourney)
...
reg_season_sample <- head_to_head_matchup_stats %>%
    ...
    select(-Team_name, -outcome_1_0, -Season, -DayNum, -TeamID, -possessions, -possessions_OPP, -TeamID_OPP, -Team_name_OPP, -Loc, -tournament_type, -tournament_type_OPP, -Team_conf, -Team_conf_OPP) %>%
...
## Random Forest: 2003-2013 (to Predict 2014-2017 Tourney)
...
regularization_grid <- expand.grid(lambda = 10 ^ seq(2, -2, length = 100))
...
#str(rf_fit_2003_2013)
# names(rf_fit_2003_2013$trainingData)
```

## Random Forest Subset1: 2003-2013 (to Predict 2014-2017 Tourney)
...
## Regularized Logistic Regression: 2003-2013 (to Predict 2014-2017 Tourney)

Try regularized logistic regression with `caret` and `glmnet`. Regularized models try to control the bias-variance tradeoff by imposing a penalty (increase the bias) on having too many predictors, in order to reduce the variance of the model. It's helpful to yield less complex models by shrinking coefficient values of predictors, which can also help reduce the multicollinearity problem in a large set of different predictors.  All my predictors are either numeric (continuous or discrete) or logical boolean in one instance. No categorical predictors, which should make it easy to transform the modeling data frame into a modeling matrix.

Hyperparameters to tune:

* LASSO regression: L1 penalization. alpha = 1 is pure LASSO regression per `glmnet` documentation. Higher values of the lambda tuning parameter **shrink** the model function's parameters towards 0. Coefficients of all predictors approach but never actually reach 0. 
* 
Ridge regression: L2 penalization is used for Ridge regression. alpha = 0 is pure Ridge regression per `glmnet` documentation. Lambda is also a tuning parameter. Difference from LASSO is that the coefficients of initial predictors can be shrunk exactly to zero, performing automatic **feature selection**. Very useful when we have many predictor variables to choose from. 
* ElasticNet: A combination of LASSO and Ridge regression. Alpha is set or tuned somewhere between 0 and 1. 
...
# setup a alpha and lambda grid to search for the pair of hyperparameters that minimize cross-validation error metric using either LASSO (alpha = 1) or Ridge (alpha = 0)
ridge_lasso_tune_grid <- expand.grid(alpha = c(0, 0.5, 1),
  lambda = seq(0.001, 0.25, length = 25))



set.seed(321)
glmnet_fit_2003_2013 <- train(x = as.matrix(modeling_dataset[, -c(1:2)]),
                 y = modeling_dataset$outcome,
                 method = 'glmnet',
                 trControl = training_setup,
                 metric = "ROC",
                 tuneGrid = ridge_lasso_tune_grid)

glmnet_fit_2003_2013
glmnet_fit_2003_2013$bestTune
plot(glmnet_fit_2003_2013)
# save model.
LASSO_fit_2003_2013 <- glmnet_fit_2003_2013$finalModel
#coef(regularization_fit$finalModel)

# model saved on 3/13/18 at 8:07pm
s3save(LASSO_fit_2003_2013, bucket = "ncaabasketball", object = "LASSO_fit_2003_2013.Rdata")
#s3load(object = "LASSO_fit_2003_2013.Rdata", bucket = "ncaabasketball")
#str(LASSO_fit_2003_2013)
# names(LASSO_fit_2003_2013$trainingData)
```


## Modeling Dataset 2003-2017 (to Predict 2018 Tourney)

I've built models on 2003-2013 data, and tested it on the 2014-2017 tournament games in the script titled **Scoring_2014_2017.R**, this showed good predictive performance (AUC of .77, log-loss of 0.57 for the random forest). Now it's time to refit the optimal model for predicting the upcoming 2018 tournament by allowing for records from 2014-2018 regular season and all records from the 2014-2017 NCAA Tournament to help train the model using more recent data. That means:

* a 25% random sample of regular season games that occurred after the 1st 25% of the regular season, and from seasons between 2003 and 2018. 
* ALL NCAA Men's tournament games from 2003 to 2017. 

``` {r}

set.seed(321)
reg_season_sample_2003_2018 <- head_to_head_matchup_stats %>%
    # drop games ineligible: 2014:2018 Season
    # games that are beginning of season have no 1-day lag summary stats. drop them.
    # also, only consider games starting after the quarter mark of the season.
    filter(complete.cases(.),
...
NCAA_tourney_2003_2017 <- head_to_head_matchup_stats %>%
    # include all NCAA tourney games from 2003 - 2013.
    filter(complete.cases(.),
           Season %in% 2003:2017, 
           tournament_type == "NCAA Tournament") %>%
...
modeling_dataset <- bind_rows(reg_season_sample_2003_2018, NCAA_tourney_2003_2017) %>%
    mutate(outcome = factor(outcome, levels = c("L", "W")))
# reorder the rows randomly.
set.seed(321)
modeling_dataset <- modeling_dataset[sample(nrow(modeling_dataset)),]
...
# nrow(modeling_dataset) == sum(nrow(reg_season_sample_2003_2018), nrow(NCAA_tourney_2003_2017))
# contrasts(modeling_dataset$outcome)
```

## Random Forest: Final Fit for Predicting 2018 Tournament. 

Try random forest model with `caret` and `randomforest`. This model was trained using over 70 predictors, and incorporates games from 2014-2017 in addition to the similar prior model built using only 2003-2013 games. 
...
# define a grid of parameter options to try in ranger
rf_grid <- expand.grid(mtry = c(1, 5, 10, 20))

set.seed(321)
rf_fit_2003_2018 <- train(x = modeling_dataset[, -c(1:2)],
                 y = modeling_dataset$outcome,
                 method = 'rf',
                 trControl = training_setup,
                 metric = "ROC",
...
rf_fit_2003_2018
names(rf_fit_2003_2018) # what objects are stored in the variable importance
varImp(rf_fit_2003_2018)
varImpPlot(rf_fit_2003_2018$finalModel)
plot(rf_fit_2003_2018)
# FINAL MODEL
#s3save(rf_fit_2003_2018, bucket = "ncaabasketball", object = "rf_fit_2003_2018.Rdata")
```

## Regularized Logistic Regression: Final Fit for Predicting 2018 Tournament. 

Create a final fit of the regularized model (LASSO) with `caret` and `glmnet` previously identified, and tested in the **Scoring_2014_2017.R** script. It showed 0.79 AUC and 0.56 log-loss test performance on the 2014-2017 tournament games. 

This model was trained using over 70 predictors, and incorporates games from 2014-2017 in addition to the similar prior model built using only 2003-2013 games. 
...
lasso_tune_grid <- expand.grid(alpha = c(1), # just set up for LASSO
  lambda = seq(0.001, 0.25, length = 25))

set.seed(321)
glmnet_fit_2003_2018 <- train(x = as.matrix(modeling_dataset[, -c(1:2)]),
                 y = modeling_dataset$outcome,
                 method = 'glmnet',
                 trControl = training_setup,
                 metric = "ROC",
...
glmnet_fit_2003_2018
glmnet_fit_2003_2018$bestTune
plot(glmnet_fit_2003_2018)
# save FINAL model
LASSO_fit_2003_2018 <- glmnet_fit_2003_2018$finalModel

# model saved on 3/13/18 at 10:15pm
s3save(LASSO_fit_2003_2018, bucket = "ncaabasketball", object = "LASSO_fit_2003_2018.Rdata")
#s3load(object = "LASSO_fit_2003_2018.Rdata", bucket = "ncaabasketball")
#str(LASSO_fit_2003_2018)
# names(LASSO_fit_2003_2018$trainingData)
```