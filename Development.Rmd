...
Some of the first things I want to create from the existing data structure of *RegularSeasonDetailedResults.csv* is to capture statistics about the opponent of the winning team, and the losing team, before I re-structure the dataset into a more tidy format downstream. Stats about opponents like 
...
In order to properly start computing lagging aggregate summary stats prior to every game, the data needs to be restructured from this very untidy wide format. Data from both teams playing a game are in one row, with similar variables (ex. FGM) spread across multiple columns. A given team's summary data will  be in the prefixed **W** columns when they won, but other times their summary data will be in the prefixed **L** columns from their losses. 

However, for good data pre-processing of team cumulative summary statistics prior to each game, I need each row to represent every unique game played by every team. 
...
Initial predictors I am considering developing represent 1-game lag averages of team's prior game performance. These will primarily focus on rate type metrics, as opposed to counting statistics. The former remove the bias of context such as playing style/tempo, which contribute towards noise in counting stats (ex. total points, total 3p made, etc). Some examples include
