 ...
 **NEXT FEATURES: ORB%, DRB%, Turnover% (offense and defense), Free Throw Fact (offense and defense) **
...
reg_season_details_long_features <- reg_season_details_long %>%
    ...
    mutate(ppp_offense = lag(cumsum(Score), 1) / lag(cumsum(possessions), 1),
           ...
           ppp_pct_diff = round(((ppp_offense / ppp_defense) -1), 1),
           avg_score_diff = lag(cummean(Score_diff), 1),
           season_wins = lag(cumsum(outcome_1_0), 1),
           season_win_rate = season_wins / lag(row_number(), 1),
           # offensive related
           assist_to_fgm_ratio = lag(cumsum(Ast), 1) / lag(cumsum(FGM), 1),
           effective_fg_pct = (lag(cumsum(FGM), 1) + lag(cumsum(0.5 * FGM3), 1)) / lag(cumsum(FGA), 1))
...
temp <- reg_season_details_long %>%
    ...
    mutate(ppp_offense = lag(cumsum(Score), 1) / lag(cumsum(possessions), 1),
           ...
           assist_to_fgm_ratio = lag(cumsum(Ast), 1) / lag(cumsum(FGM), 1),
           effective_fg_pct = (lag(cumsum(FGM), 1) + lag(cumsum(0.5 * FGM3), 1)) / lag(cumsum(FGA), 1)) %>%
    select(1:6, possessions, Ast, FGM, FGM3, FGA, assist_to_fgm_ratio, effective_fg_pct, season_wins, season_win_rate)
