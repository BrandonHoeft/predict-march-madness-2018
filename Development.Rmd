...
reg_season_details_long_features <- reg_season_details_long %>%
    mutate(possessions = 0.96 * ((FGA) - (OR) + (TO) + (0.44 * FTA))) %>% # https://www.nbastuffer.com/analytics101/possession/
    ...
    # 1 game lag cumulative or other aggregated team stats, within season, per team.
    mutate(cum_ppp_offense = lag(cumsum(Score), 1) / lag(cumsum(possessions), 1),
           cum_ppp_defense = lag(cumsum(Score - Score_diff), 1) / lag(cumsum(possessions), 1),
           # cumulative % difference in offense vs. defensive ppp
           cum_ppp_pct_diff = round(100 * ((cum_ppp_offense / cum_ppp_defense) -1), 1),
           avg_score_diff = lag(cummean(Score_diff), 1),
           cum_season_wins = lag(cumsum(outcome_1_0), 1),
           cum_season_win_rate = cum_season_wins / lag(row_number()), 1)
...
temp <- reg_season_details_long %>%
    ...
    arrange(Team_name, Season, DayNum) %>%
    # 1 game lag cumulative or other aggregated team stats, within season, per team.
    mutate(ppp_offense = lag(cumsum(Score), 1) / lag(cumsum(possessions), 1),
           ppp_defense = lag(cumsum(Score - Score_diff), 1) / lag(cumsum(possessions), 1),
           # cumulative % difference in offense vs. defensive ppp
           ppp_pct_diff = round(100 * ((cum_ppp_offense / cum_ppp_defense) -1), 1),
           avg_score_diff = lag(cummean(Score_diff), 1),
           season_wins = lag(cumsum(outcome_1_0), 1),
           season_win_rate = cum_season_wins / lag(row_number(), 1),
           # offensive related
           assist_to_fgm_ratio = lag()) %>%
    select(1:6, possessions, Score_diff, avg_score_diff, cum_ppp_offense, cum_ppp_defense, cum_ppp_pct_diff, cum_season_wins, cum_season_win_rate)
    

temp2 <- temp %>%
    filter(Season == 2017 & DayNum %in% 11:117) %>%
    summarize(ppp_offense = sum(Score) / sum(possessions))
```
