...
reg_season_details_cumulative_stats <- reg_season_details_long %>%
    ...
    mutate(possessions = lag(cummean(possessions)), # default lag of n = 1 prior period.
...
#calculate here          season_wins = lag(cumsum(outcome_1_0)),
#calculate here           season_win_rate = season_wins / lag(row_number()),
           # more offens related
           ...
           # Turnover factor: turnovers per 100 possessions.
           offense_tov_per_100 = 100 *(lag(cumsum(TO)) / (lag(cumsum(FGA)) + lag(cumsum(0.44 * FTA)) + lag(cumsum(TO))))
           ) %>%
    select(-possessions)
