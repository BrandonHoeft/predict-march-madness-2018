...
Since the RankingDayNum does not match exactly with the DayNum from the box score data, a grid needs to be built that has every combination of possible TeamID, Season, DayNum (1-133). Then for each DayNum of a TeamID's Season, we can replace DayNum rows that don't have a team rating as of the specific RankingDayNum, by replacing this missing value with the latest non-missing value using the `zoo` library.
...
head_to_head_stats <- reg_season_details_cumulative_stats %>%
    ...
    # Select variables
    select(game_id, team_season_id, Season, DayNum, TeamID, Team_name, Team_conf, outcome, outcome_1_0,
           Loc, possessions, offense_efficiency, defense_efficiency,
...
``` {r}
set.seed(321)
modeling_dataset <- head_to_head_matchup_stats %>%
    ...
    filter(complete.cases(.),
           DayNum > quantile(RegularSeasonDetailedResults$DayNum, .25)) %>%
    select(-Team_name, -outcome_1_0, -Season, -DayNum, -TeamID, -possessions, -possessions_OPP, -TeamID_OPP, -Team_name_OPP) %>%
...
modeling_dataset <- select(modeling_dataset, -game_id) %>%
    mutate(outcome = factor(outcome, levels = c("L", "W")),
           Team_conf = factor(Team_conf),
           Loc = factor(Loc),
           Team_conf_OPP =  factor(Team_conf_OPP))

modeling_dataset_no_conf <- select(modeling_dataset, -Team_conf, -Team_conf_OPP)
...
Try random forest model with `caret` and `randomforest`.
...
# define a grid of parameter options to try in ranger
rf_grid2 <- expand.grid(mtry = c(1, 5, 10, 15, 25))

set.seed(321)
rf_fit2 <- train(x = modeling_dataset[, -1],
                 y = modeling_dataset$outcome,
                 method = 'rf',
                 trControl = training_setup,
                 metric = "ROC",
...
rf_fit2
names(rf_fit2) # what objects are stored in the variable importance
varImp(rf_fit2)
```

## Random Forest 

Try random forest model with `caret` and `randomforest`. Don't use team conference or opponent's team conference as a predictor. This leads to the best model so far with mtry = 25 and 5-fold AUC of 0.792.

```{r}
set.seed(321)
rf_fit3 <- train(x = modeling_dataset_no_conf[, -1],
                 y = modeling_dataset_no_conf$outcome,
                 method = 'rf',
                 trControl = training_setup,
                 metric = "ROC",
                 tuneGrid = rf_grid2)
...
rf_fit3
names(rf_fit3) # what objects are stored in the variable importance
varImp(rf_fit3)
```
...
xgb_training_setup <- trainControl(method = "cv",
                               number = 5,
                               savePredictions = TRUE,
                               summaryFunction = twoClassSummary, # needed for ROC metric
                               classProbs = TRUE,
...
# define a grid of parameter options to try in ranger
xgb_grid_1 <- expand.grid(nrounds= c(50,100,200,300),
                          lambda = c(0),
                          alpha = c(1))

xgb_train_1 <- train(
  x = as.matrix(modeling_dataset[, -1]),
  y= df_train$Response,
...
NCAA_tourney_13_17_winners <- NCAATourneyDetailedResults %>% 
    filter(Season %in% 2014:2017) %>%
    # see head to head stats, line 289 for identifier dataset. 
    mutate(team_season_id = paste(Season, WTeamID, sep = "_"),
           outcome = "W") %>%
    select(team_season_id, Season, DayNum, outcome, starts_with("W")) %>%
    rename_at(vars(starts_with("W")), # for columns starting with W
              function(x) stringr::str_sub(x, start = 2L, end = -1L)) %>% # strip W off column name.
    select(team_season_id, Season, DayNum, TeamID, outcome)

NCAA_tourney_13_17_losers <- NCAATourneyDetailedResults %>% 
    filter(Season %in% 2014:2017) %>%
    # see head to head stats, line 289 for identifier dataset. 
    mutate(team_season_id = paste(Season, WTeamID, sep = "_"),
           outcome = "L") %>%
    select(team_season_id, Season, DayNum, outcome, starts_with("L")) %>%
    rename_at(vars(starts_with("L")), # for columns starting with W
              function(x) stringr::str_sub(x, start = 2L, end = -1L)) %>% # strip L off column name.
    select(team_season_id, Season, DayNum, TeamID, outcome)
